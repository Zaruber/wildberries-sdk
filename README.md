...
- **Валидация request/response.** Обязательные поля, форматы (uuid, date-time), enum-значения и ограничения (min/max) проверяются на этапе сериализации, а не в рантайме на сервере.
